...
Update Frequency & Approximately 1 Hz (downsampled) \\
Internal Propagation Latency (typical) & 80--150 microseconds \\
Network Latency (exchange to handler) & Variable (measured) \\
History Buffer & 3 hours at 1Hz \\
...
\begin{itemize}
    ...
    \item \textbf{parking\_lot}: High-performance reader-writer locks for shared state
\end{itemize}

\subsection{Module Structure}

The key source files are located in \texttt{rust-backend/src/scrapers/}:
\begin{itemize}
    \item \texttt{binance\_price\_feed.rs} -- Primary L1 mid-price feed
    \item \texttt{binance\_arb\_feed.rs} -- Enhanced feed with trades and OHLC
    \item \texttt{chainlink\_feed.rs} -- Oracle comparison tracker
    \item \texttt{oracle\_comparison.rs} -- Binance/Chainlink divergence analysis
\end{itemize}
...
Typical differential: 80 $\mu$s vs 150--200 $\mu$s, providing a 70--120 $\mu$s informational edge.
...
if let (Some(prev_mid), Some(prev_ts)) = 
    (entry.last_mid, entry.last_ts) {
    let dt = (ts - prev_ts).max(1) as f64;
    if prev_mid > 0.0 && mid > 0.0 {
        let r = (mid / prev_mid).ln() / dt;  // Log return per sec
        let var_obs = r * r;
...
\begin{itemize}
    \item \texttt{GET /api/performance/latency} returns \texttt{SystemLatencySnapshot}
    \item \texttt{GET /api/binance/arb/snapshot} returns \texttt{ArbSnapshot} with latency history
\end{itemize}
...
The latency differential between Binance (approximately 80 $\mu$s) and Polymarket (approximately 150--200 $\mu$s) provides the informational edge exploited by the FAST15M strategy for 15-minute Up/Down market arbitrage.
