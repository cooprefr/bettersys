#!/bin/bash
# Binance Latency Probe Bootstrap Script
# Terraform template variables are interpolated at deploy time
...
LOG_FILE="/var/log/latency-probe-bootstrap.log"
...
dnf install -y gcc openssl-devel pkg-config git
...
# Pin smol_str to avoid Rust 1.89 requirement
smol_str = "=0.3.2"
axum = "0.7"
tower = "0.4"
...
impl LatencyHistogram {
    ...
    fn record(&self, latency_us: u64) {
        ...
        loop {
            ...
            if self.min_us.compare_exchange_weak(current_min, latency_us, Ordering::Relaxed, Ordering::Relaxed).is_ok() { break; }
        }

        loop {
            let current_max = self.max_us.load(Ordering::Relaxed);
            if latency_us <= current_max { break; }
            if self.max_us.compare_exchange_weak(current_max, latency_us, Ordering::Relaxed, Ordering::Relaxed).is_ok() { break; }
        }

        let idx = BUCKET_BOUNDS_US.iter().position(|&bound| latency_us <= bound).unwrap_or(BUCKET_BOUNDS_US.len() - 1);
    ...
    fn percentile(&self, p: f64) -> u64 {
        ...
        for (i, bucket) in self.buckets.iter().enumerate() {
            ...
            if cumulative >= target { return BUCKET_BOUNDS_US[i]; }
        }
...
impl ProbeState {
    fn new(args: Args) -> Self {
        ...
        let histograms: HashMap<String, Arc<LatencyHistogram>> = symbols.iter().map(|s| (s.clone(), Arc::new(LatencyHistogram::new()))).collect();
        Self {
            args,
    ...
    fn is_warmed_up(&self) -> bool { self.warmup_complete.load(Ordering::Relaxed) > 0 }
    ...
    fn record_latency(&self, symbol: &str, exchange_ts_ms: i64, receive_ts_ms: i64) {
        ...
        if let Some(hist) = self.histograms.get(symbol) { hist.record(latency_us); }
        ...
        if recent.len() >= 1000 { recent.remove(0); }
        recent.push(sample);
    }
}
...
async fn get_metrics(axum::extract::State(state): axum::extract::State<Arc<ProbeState>>) -> Json<MetricsResponse> {
    let per_symbol: HashMap<String, HistogramSummary> = state.histograms.iter().map(|(k, v)| (k.clone(), v.summary())).collect();
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::from_default_env().add_directive("binance_latency_probe=info".parse().unwrap()))
        .json()
        .init();
    ...
    tokio::spawn(async move {
        ...
        let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", metrics_port)).await.expect("Failed to bind metrics port");
        ...
        match event {
            ...
            ReconnectEvent::Item(result) => match result {
                Ok(market_event) => {
                    ...
                    let symbol = format!("{}{}", market_event.instrument.base, market_event.instrument.quote).to_uppercase();
...
async fn init_streams() -> Result<Streams<barter_data::streams::consumer::MarketStreamResult<MarketDataInstrument, barter_data::subscription::book::OrderBookL1>>> {
...
echo "Building probe binary (this may take a few minutes)..."
cargo build --release
...
Environment="PROBE_WARMUP_SEC=$WARMUP_SEC"
Environment="PROBE_METRICS_PORT=$METRICS_PORT"
Environment="RUST_LOG=info"
ExecStart=$PROBE_DIR/target/release/binance-latency-probe
Restart=always
RestartSec=5
...
echo "Service status:"
systemctl status binance-probe --no-pager || true
echo ""
echo "Metrics endpoint: http://$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4):$METRICS_PORT/metrics"
